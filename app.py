...
# Comment out template routes to use React app instead
# @app.route('/models')
# def models():
...
def serve_react_assets(filename):
    ...
    try:
        return send_file(os.path.join('static/frontend/dist/assets', filename))
    except Exception as e:
        print(f"Error serving React asset {filename}: {e}")
        return "Asset not found", 404

# Serve React App static files from root path
@app.route('/assets/<path:filename>')
def serve_react_assets_root(filename):
    """Serve React app static assets from root path"""
    try:
        return send_file(os.path.join('static/frontend/dist/assets', filename))
    except Exception as e:
        print(f"Error serving React asset {filename}: {e}")
...
def serve_static_files():
    """Serve static files like favicon, manifest, etc."""
    filename = request.path[1:]  # Remove leading slash
    try:
        return send_file(os.path.join('static/frontend/dist', filename))
    except Exception as e:
        print(f"Error serving static file {filename}: {e}")
        return "File not found", 404

# Serve React App for all frontend routes - MUST come before template routes
@app.route('/api')
@app.route('/api/')
...
def serve_react_routes(path=''):
    ...
    try:
        # Always serve the main index.html for React Router
        return send_file('static/frontend/dist/index.html')
    except Exception as e:
        print(f"Error serving React route: {e}")
        return "React app not found. Please build the frontend first.", 404

# Serve React App for root and other paths
@app.route('/')
@app.route('/<path:path>')
def serve_react_app(path=''):
    """Serve the React application"""
    # Don't serve React app for API routes and static files
    if path.startswith('api/') or path.startswith('download/') or path.startswith('generate-api-key'):
    ...
    try:
        ...
        if '.' in path and not path.endswith('.html'):
            try:
                return send_file(f'static/frontend/dist/{path}')
            except:
                pass
        ...
        print(f"Error serving route {path}: {e}")
        return "React app not found. Please build the frontend first.", 404
